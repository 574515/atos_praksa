...
public class Source {
	...
	public static void main(String[] args) {
		...
		for(i = 0; i < 6; i++) {
			if (i == 0) prices[i] = 4500;
			else prices[i] = increment -= 1000;
		}
	...
	public static void xPositionOnTable(int[] electricityBill, int price) {
		...
		int i, j;
		for(i = 0; i <= 11; i++) monthPosition[i] = i * 3;
		i = j = 0;
	...
	public static void getBillValue(int[] electricityBill) {
		for(int i = 0; i < 12; i++) {
			if(electricityBill[i] <= 500) electricityBill[i] = 0;
			else if(electricityBill[i] > 500 && electricityBill[i] <= 1500)	electricityBill[i] = 1000;
			else if(electricityBill[i] > 1500 && electricityBill[i] <= 2500) electricityBill[i] = 2000;
			else if(electricityBill[i] > 2500 && electricityBill[i] <= 3500) electricityBill[i] = 3000;
			else if(electricityBill[i] > 3500 && electricityBill[i] <4500 ) electricityBill[i] = 4000;
			else if(electricityBill[i] >= 4500) electricityBill[i] = 4500;
		}
	}
	...
	private static int[] getRandomElectricityBill() {
		...
		for(i = 0; i < months; i++) electricityBill[i] = rand.nextInt(5000);
